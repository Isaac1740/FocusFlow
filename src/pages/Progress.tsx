...
// default radar data if needed
function defaultRadar(focusScore: number, dsaScore: number) {
  return [
    { category: "Focus", value: focusScore },
    { category: "DSA", value: dsaScore },
    { category: "Planning", value: 75 },
    { category: "Consistency", value: 60 },
    ...
    async function load() {
      ...
      try {
        ...
        for (const day of days) {
          ...
          const url = `${API_URL}/api/get_tasks?user_id=${encodeURIComponent(
            userId
          ...
          try {
            ...
            const tasks: Task[] =
              json && json.success && Array.isArray(json.tasks) ? json.tasks : [];
            ...
            const dayDSA = tasks.filter((t) =>
              /dsa|leetcode|problem|algo|algorithm|practice|code/i.test(t.task)
            ).length;
        ...
        const productivityPercent = Math.round(
          (daysWithTasks.length / days.length) * 100
        );
        ...
        const focusScore = Math.min(
          100,
          Math.round((hours / (7 * 2)) * 100)
        );

        const totalTasksCount =
          perDay.reduce(
            (acc, d) => acc + (d.focus > 0 ? 1 : 0) + d.dsa,
            0
          ) || 0;
        const totalDSA = perDay.reduce((acc, d) => acc + d.dsa, 0);
        const dsaScore = Math.min(
          100,
          Math.round((totalDSA / Math.max(1, totalTasksCount)) * 100)
        );
    ...
    <div className="min-h-screen p-6 md:p-8 max-w-6xl mx-auto bg-background transition-colors duration-300">
      ...
      >
        ...
        <p className="text-muted-foreground">
          Your real progress (last 7 days)
        </p>
      </motion.div>
                ...
                <DialogContent className="glass-card border-border">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Slider
                        value={[newTracker.progress]}
                        onValueChange={(v) =>
                          setNewTracker({ ...newTracker, progress: v[0] })
                        }
                        max={100}
                      />
                    </div>
            ...
            <div className="grid gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        variant="ghost"
                        onClick={() =>
                          updateTracker(t.id, Math.min(100, t.progress + 10))
                        }
                      >
                        +
                      </Button>
              ...
              {customTrackers.length === 0 && (
                <div className="glass-card rounded-2xl p-8 text-center">
                  No custom trackers yet. Create one to track your goals!
                </div>
              )}
            </div>
          </div>
