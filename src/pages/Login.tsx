...
const Login: React.FC = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 transition-colors duration-500">
      <button onClick={toggleTheme} className="absolute top-5 right-5 bg-white/20 dark:bg-gray-800 text-white p-2 rounded-full shadow-md hover:scale-105 transition">🌗</button>
      <div className="backdrop-blur-lg bg-white/20 dark:bg-gray-800/40 p-10 rounded-2xl shadow-2xl w-full max-w-md text-white border border-white/20">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button type="submit" disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white font-semibold py-2 rounded-lg transition disabled:opacity-60">
            {loading ? "Logging in..." : "Log In"}
          </button>
        </form>
        <p className="text-center text-sm mt-6 text-white/80">
          Don’t have an account? <a href="/signup" className="text-indigo-300 hover:text-indigo-100 underline transition">Sign up</a>
        </p>
      </div>
    </div>
  );
